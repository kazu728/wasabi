...
#[test_case]
fn test_round_up_to_nearest_pow2() {
...
// 4KB のメモリ領域を、4KB 境界に揃えて確保したいときの条件
// size: 必要なメモリの大きさ
// align: 先頭アドレスを何バイト境界に揃えるか
...
impl Header {
    ...
    // > All layouts have an associated size adn a power-of-two alignment
    fn provide(&mut self, size: usize, align: usize) -> Option<*mut u8> {
        ...
        if self.is_allocated() || !self.can_provide(size, align) {
            ...
            // Each char represents 32-bytes chunks
            let mut size_used = 0;

            // size バイト分確保するときの開始アドレスを、align に合うように下方向へ丸める
            ...
            // 確保したチャンクが元のチャンクの末尾まで届いておらず余りがある場合はpadding追加
...
impl Drop for Header {
    fn drop(&mut self) {
        panic!("Hadnler should not be dropped")
    }
}
...
impl FirstFitAllocator {
    ...
    pub fn init_with_nmap(&self, memory_map: &MemoryMapHolder) {
...
mod tests {
    ...
    fn dump_free_list() {
        let headers = ALLOCATOR.first_header.borrow();
        let mut current = headers.as_deref();
        let mut writer = SerialPort::new_for_com1();

        writeln!(writer, "allocator free list dump:").unwrap();

        let mut index = 0;
        while let Some(header) = current {
            writeln!(
                writer,
                "  hdr[{index}] addr={:#018X} size={:#018X} allocated={} end={:#018X}",
                header as *const Header as usize,
                header.size,
                header.is_allocated,
                header.end_addr(),
            )
            .unwrap();
            current = header.next_header.as_deref();
            index += 1;
        }

        if index == 0 {
            writeln!(writer, "  <empty>").unwrap();
        }
    }
    ...
    #[test_case]
    fn malloc_align() {
        // アラインメントを大きくしながらallocした際にも正しく動作する
    ...
    #[test_case]
    fn allocated_objetcs_no_overlap() {
        // 確保した領域が重複していないこと
        let allocations = [
        ...
        let mut pointers = alloc::vec![null_mut::<u8>(); allocations.len()];
        ...
        for e in allocations.iter().zip(pointers.iter()).enumerate() {
            let (i, (layout, pointer)) = e;
            for k in 0..layout.size() {
                assert_eq!(unsafe { *pointer.add(k) }, i as u8);
            }
        }
