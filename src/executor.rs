...
use crate::info;
use crate::{result::Result, x86_64::busy_loop_hint};
use alloc::boxed::Box;
use alloc::collections::VecDeque;
use core::panic::Location;

pub struct Task<T> {
    // // async fn が作る Future は、await をまたいで内部状態を持つので、メモリアドレスが固定されている前提が必要
    future: Pin<Box<dyn Future<Output = Result<T>>>>,
    created_file: &'static str,
    created_line: u32,
}
...
impl<T> Task<T> {
    ...
    pub fn new(future: impl Future<Output = Result<T>> + 'static) -> Task<T> {
        Self {
            ...
            created_file: Location::caller().file(),
            created_line: Location::caller().line(),
        }
    }
...
impl<T> Debug for Task<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "Task ({}:{})", self.created_file, self.created_line)
    }
}
    ...
    pub fn run(mut executor: Self) -> ! {
        ...
        loop {
            ...
            if let Some(mut task) = task {
                ...
                match task.poll(&mut context) {
                    Poll::Ready(result) => info!("Task completed: {:?}: {:?}", task, result),
                    Poll::Pending => executor.task_queue().push_back(task),
                }
            }
        }
    }
