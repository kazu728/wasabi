...
pub struct HpetRegisters {
    capabilities_and_id: u64, // カウンタ周期やタイマー数などの情報を持つ。
    _reserved0: u64,
    configuration: u64, //  HPET 全体の有効/無効などを切り替える。
    _reserved1: [u64; 27],
    main_counter_value: u64, //単調増加するメインカウンタ本体。周期は capabilities_and_id の下位 32 ビットで指定される。
    _reserved2: u64,
    timers: [TimerRegister; 32], // 0x100-: 各タイマーの 0x20 バイトブロック。この実装では各ブロック先頭の設定レジスタだけを扱う。
}

const _: () = assert!(core::mem::size_of::<HpetRegisters>() == 0x500);
...
impl Hpet {
    pub fn new(registers: &'static mut HpetRegisters) -> Self {
        ...
        let freq = 1_000_000_000_000_000 / fs_per_count; // HPET の周期(fs/count)から、周波数(counts/sec)を求める
        let mut hpet = Self {
            registers,
            num_of_timers,
            freq,
